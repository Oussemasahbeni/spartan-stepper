...
import { HlmFieldImports } from '../libs/field/src';
import { HlmInputImports } from '../libs/input/src';
import { HlmStepperImports } from '../stepper';

@Component({
  selector: 'docs-stepper-error-example',
  imports: [
    ReactiveFormsModule,
    HlmStepperImports,
    HlmButtonImports,
      ...
      >
        <form [formGroup]="contactForm" class="flex flex-col gap-4">
          ...
          <p class="text-sm text-muted-foreground">
            This example is intentionally non-linear. Click next without filling the field to move
            forward and trigger the header error state.
          </p>
        ...
        <div class="flex flex-col gap-4">
          ...
          >
            <p class="max-w-sm text-center text-sm text-muted-foreground">
              Because <code class="font-mono text-foreground">showError</code> is enabled through
              <code class="font-mono text-foreground">STEPPER_GLOBAL_OPTIONS</code>, the previous
              step now displays its error message in the header.
            </p>
          </div>
...
export class StepperErrorExampleComponent {
